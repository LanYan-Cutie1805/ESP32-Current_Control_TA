...
const float TEMP_TRIP = 150.0f;
const float TEMP_RST  = 75.5f;
...
void reconnect() {
  ...
  client.subscribe("ICCP/002/control");
  client.subscribe("ICCP/002/setpoint");
}
...
float readVW() {
  ...
  float adc_avg = (float)sum / OVERSAMPLE;

  float v_gpio = (adc_avg / ADC_MAX) * ADC_REF;

  float real_voltage =
      applyCalibration(v_gpio) * DIVIDER_RATIO;
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // ===== CONTROL MODE =====
    ...
    // ===== MANUAL SETPOINT =====
if (String(topic) == "ICCP/002/setpoint") {

  if (controlMode == MODE_MANUAL) {

    float req = message.toFloat();
    ...
    if (req >= MIN_VOLTAGE && req <= MAX_VOLTAGE) {
      ...
      Serial.print("MQTT Setpoint: ");
      Serial.println(targetVoltage);

    } else {

      Serial.println("Invalid MQTT setpoint");
    }
  }
}
}
...
void loop() {
  ...
  if (controlMode == MODE_MANUAL) {
    ...
    int targetStep = constrain(targetVoltage * 10, 0, MAX_CAL_STEP);
  ...
  float ref_v = readADS_Ref();
  float reff_v = ref_v * -1;
  float voltage_inj = ina.getBusVoltage_mV(); // convert mV → V
  float current_inj = ina.getCurrent_mA();

  float current  = measureCurrent();
  float real_voltage = readVW() * 1000;

  // modif
  float im_voltage = constrain(real_voltage, 0.0f, 2000.0f);
  float current_i = im_voltage - 0.023;
  float bus_v = (current_i/1000) * 40.09367;
  
  if (controlMode == MODE_AUTO) {

    float ads_mV = - (readADS_Ref() * 1000.0);
  ...
  doc["bus_voltage_V"] = bus_v; 
  doc["current_mA"] = current_i;
  doc["target_current_mA"] = real_voltage;
  doc["electrode_V"]   = reff_v;
  doc["Temp (C)"]        = lastTemp;
  doc["Relay"] = relayStateStr();

  char payload[200];
  serializeJson(doc, payload);

  String combinedTopic = topic + "/data";
  client.publish("ICCP/002/current", 1, false, payload); 
  bool ok = client.publish(combinedTopic.c_str(), payload);
  delay(1000);
  }