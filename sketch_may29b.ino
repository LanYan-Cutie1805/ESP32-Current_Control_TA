...
float measureCurrent() {
  ...
  return (current < 0.01f) ? 0.0f : current;
}
...
void loop() {
  ...
  float real_voltage = readVW();

  // modif
  float im_voltage = constrain(real_voltage, 0.0f, 2.0f);
  ...
  doc["Target injection (A)"] = im_voltage;
  doc["Injection current (mA)"] = current_i;
  doc["Rectifier Current (A)"] = current;
  doc["Electrode (mV)"]   = lastVRef;
  doc["Temp (C)"]        = lastTemp;
